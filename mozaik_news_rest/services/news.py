...
class NewsService(Component):
    ...
    def _search_filter_news(self, filters: NewsInputFilter):
        domain = []
        if filters.start_date_at_least:
            domain.append(("start_date", ">=", filters.start_date_at_least))
        if filters.end_date_at_most:
            domain.append(
                ("end_date", "!=", False), ("end_date", "<=", filters.end_date_at_most)
            )
